...
async function main() {
  ...
  for (const kb of specAny.knowledgeBases ?? []) {
    for (const ds of kb.dataSources ?? []) {
      if (ds.type !== 'S3' && ds.connectorConfigFile) {
        ...
        try {
          connectorParametersByFile[ds.connectorConfigFile] = JSON.parse(
  ...
  for (const target of targets) {
    ...
    const targetState = (deployedState as Record<string, unknown>)?.targets as
      ...
      | undefined
    const targetResources = targetState?.[target.name]?.resources as
          ...
          (p: {
            ...
            connectors: p.connectors.map(c => {
              const credentialProviderArn =
                paymentCredentials?.[c.credentialName]?.credentialProviderArn
