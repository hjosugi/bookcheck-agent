...
**Phase 1 から Phase 8 まで、最初の Google Cloud 設定から片付けまでを通してたどれます。**
本の代わりではなく、本を開きながら「今どこにいるか」を確認するための地図として使ってください。
長いコード本文（`main.py` と `page.tsx`）だけは書籍と公式リポジトリを参照します。
...
作業時間の目安は 4〜5 時間です。分割するなら Phase 1 の後（Google 側の準備が済んだ状態）、
Phase 3 の後（バックエンドが動く状態）、Phase 5 の後（Web アプリが表示される状態）で
区切ると再開しやすいです。
...
- [ ] Google アカウント（カレンダー登録先。Google Cloud の設定は Phase 1 でやります）

> 13.1〜13.2 にあたる Google 側の準備は、このガイドの Phase 1 に書き起こしてあります。
> 書籍を開かなくても Phase 1 から順に進められます。
...
## Phase 1　Google 連携の下ごしらえ（13.1〜13.2）

所要 30〜40 分。ここはブラウザ作業だけで、コードは書きません。

### なぜ先にこれをやるのか

エージェントは AWS の中で動きますが、書き込む先は**ユーザー個人の Google カレンダー**です。
AWS の IAM 権限では届きません。ユーザー本人が Google の画面で許可する必要があります。

![3LO 認可の流れ](img/03-3lo-sequence.svg)

この「本人に許可を取り、預かったトークンをエージェントに渡す」役目を担うのが
**AgentCore アイデンティティ**です。Google のクライアント ID とシークレットを
先に預けておくと、エージェント側のコードは `@requires_access_token` を付けるだけで済みます。

つまり Phase 1 のゴールは 2 つです。

1. Google 側に「このアプリがカレンダーを触ってよい」という OAuth クライアントを作る
2. その ID とシークレットを AgentCore に登録し、**プロバイダー名**を手に入れる（メモ #1）

メモ #1 は Phase 6-2 でランタイムの環境変数 `CREDENTIAL_PROVIDER_NAME` になります。

### 1-1. Google Cloud プロジェクトを作る

<https://console.cloud.google.com/> を開きます。Google アカウントでログインし、
初回なら利用規約に同意します。**課金の有効化は不要**です。

画面上部のプロジェクト選択メニュー →「新しいプロジェクト」→
プロジェクト名を `bookchecker` にして「作成」。

作成が終わったら、画面上部が `bookchecker` になっていることを必ず確認してください。
別プロジェクトのまま進めるのが最初のつまずきどころです。

### 1-2. Google Calendar API を有効にする

左メニュー「APIとサービス」→「ライブラリ」→ `Google Calendar API` を検索 →「有効にする」。

これを忘れると、Google の認可自体は通るのにカレンダー登録の瞬間だけ
403 が返る、という切り分けの難しい失敗になります。

### 1-3. OAuth 同意画面を設定する

「APIとサービス」→「OAuth 同意画面」。新しい UI では左メニューが
「Google Auth Platform」になっていて、同じ内容を「ブランディング」「対象」「データアクセス」に
分けて設定します。
...
続けて 2 つ、忘れやすい設定があります。

- **スコープ**（データアクセス）: 「スコープを追加または削除」→
  `https://www.googleapis.com/auth/calendar.events` を探してチェック → 更新。
  これは `agent/app/BookChecker/calendar_tool.py` の `CALENDAR_SCOPES` と同じ値です。
- **テストユーザー**（対象）: 自分の Google アカウントを追加。
  公開ステータスは「テスト」のままで構いませんが、テストユーザーに入っていないアカウントは
  認可画面で `access_denied` になります。カレンダーを登録したいアカウントを入れてください。

### 1-4. OAuth クライアント ID を作る
...
「承認済みのリダイレクト URI」は、この時点ではまだ分かりません。
次の 1-5 で AgentCore 側の URI が表示されるので、**空のまま「作成」して構いません**。

作成すると **クライアント ID** と **クライアントシークレット** が表示されます。
両方ともメモ帳（[handson-memo.txt](handson-memo.txt) の Phase 1 欄）に控えてください。

> シークレットはこのダイアログを閉じると再表示できません。
> 控え忘れたら、同じクライアントの編集画面から新しいシークレットを発行し直せます。

### 1-5. AgentCore にクレデンシャルプロバイダーを登録する（13.2.1）

AWS マネジメントコンソールを開き、リージョンが**バージニア北部 `us-east-1`** であることを
確認します。Amazon Bedrock AgentCore →「アイデンティティ」→
「OAuth クライアント/API キー」→「OAuth クライアントを追加」。
...
名前は自由に決められますが、**あとで環境変数に一字一句そのまま貼る**ので、
記号やスペースを入れず、控えた文字列と完全に一致させてください。

作成すると、**Google 側に登録すべきリダイレクト URI** が画面に表示されます。
`https://bedrock-agentcore.us-east-1.amazonaws.com/identities/oauth2/callback` のような形ですが、
**表示された文字列をそのままコピー**してください。画面の値が正です。

### 1-6. Google 側にリダイレクト URI を登録する

Google Cloud の「APIとサービス」→「認証情報」→ 1-4 で作った
`bookchecker-agentcore` を開く →「承認済みのリダイレクト URI」→「URI を追加」→
1-5 でコピーした URI を貼って「保存」。

反映に数分かかることがあります。Phase 7 で Google 連携がうまくいかないときは、
まずここが保存されているかを見てください。

> この URI と、Phase 5 以降に出てくる**コールバック URL（メモ #5）は別物**です。
> メモ #5 は Amplify で公開した自分のアプリの `/api/oauth2/callback` で、
> 登録先は「ランタイム環境変数」と「ワークロード ID」の 2 か所。
> 一方この 1-6 の URI は AgentCore 自身のもので、登録先は Google だけです。
> 混同すると Phase 6 で必ず詰まります。

**チェックポイント**

- [ ] Google Cloud プロジェクト `bookchecker` ができている
- [ ] Google Calendar API が「有効」になっている
- [ ] OAuth 同意画面のスコープに `calendar.events` がある
- [ ] テストユーザーに、カレンダーを登録したい Google アカウントが入っている
- [ ] クライアント ID とシークレットをメモした
- [ ] AgentCore アイデンティティにプロバイダーができ、その**名前をメモ #1 に控えた**
- [ ] Google の承認済みリダイレクト URI に、AgentCore が示した URI を保存した
...
ここは新しい作業ではなく、[Phase 1-6](#1-6-google-側にリダイレクト-uri-を登録する) の確認です。
Google Cloud の「認証情報」→ `bookchecker-agentcore` を開き、
「承認済みのリダイレクト URI」に AgentCore が示した URI が保存されているかを見てください。

ここに入れるのは AgentCore 自身の URI です。メモ #5（自分の Amplify ドメイン + `/api/oauth2/callback`）を
Google 側に入れてしまう取り違えが多いので注意してください。
